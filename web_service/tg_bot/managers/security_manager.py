...
from tg_bot.config import FLOOD_CONTROL_TIME, FLOOD_CONTROL_STOP_TIME, FLOOD_CONTROL_NUM_ALERTS
...
class SecurityManager:
    ...
    async def check_user(
            self, update: Union[CallbackQuery, Message], user_data: Optional[Dict]) -> bool:
        ...
        if user_status == 'block':
            if settings.DEBUG:
                self.logger.warning(self.sign + f'user_status -> {user_status.upper()} '
                                                f'<- | {username=} | {user_id=}')
            result = False
        else:
            if settings.DEBUG:
                self.logger.info(self.sign + f'user_status -> {user_status.upper()} '
                                             f'<- | {username=} | {user_id=}')
            result = True
